...
\begin{lem}
	...
	\begin{align*}
		\begin{array}{lrcl}
			...
			&\psi_1							& \mapsto		&  \left( 2  v | \nabla (u)|^2  - \nu \left(  \epsilon_2 2 \Delta v - \frac{2}{\epsilon_2} (1-v),\psi_1 \right) \right)_{L^2(\Omega)} + \left( 2 \epsilon_2 \nu \nabla v \nu ,\psi_1 \right)_{L^2(\partial \Omega)}  \\
		\end{array}	
	\end{align*}
\end{lem}
\begin{proof}
	...
	\begin{align*}
		\begin{array}{lcrl}
			...
			& & &\hspace{-2ex} - \int\limits_{\Omega}  \left( v^2 + \epsilon_1 \right) | \nabla u|^2 \hspace{8ex} + \nu \left( \epsilon_2 |\nabla v|^2 \hspace{6.5ex}+ \frac{1}{\epsilon_2} \left( 1- v \right)^2 \right) \Big)  \\
			& = &\lim\limits_{t \rightarrow 0} \frac{1}{t} 
			\Big(	& 
			...
			& = &\hspace{6.5ex} & 
			\int\limits_{\Omega} 2  v | \nabla u|^2 \psi_1 - \nu \left( \epsilon_2 2 \Delta v  \psi_1 - \frac{2}{\epsilon_2} (1-v)\psi_1 \right) \diff x + \int\limits_{\partial \Omega} 2 \nu  \epsilon_2 \nabla v \nu \psi_1 \diff x  							 
		\end{array}
	\end{align*}
...
\begin{align*}
 	& \int\limits_{\Omega} 2 \psi_1  v | \nabla u|^2 + \epsilon_2 2 \nabla v \nabla \psi_1  - \frac{2}{\epsilon_2} (1-v)\psi_1 + \eta \psi_1 \diff x = 0 & \forall \psi_1 \in \h \\
...
\end{thm}
\begin{proof}
	Nach \ref{lem:semidiffbar_f_diffbar} ist $G_1$ $\partial G_1$ semidifferenzierbar, falls $G_1$ stetig Fréchet differenzierbar ist. Bestimmen wir zunächst die Richtungsableitung in Richtung $\phi_1$ bzw $\phi_2$. Diese ist gegeben durch 
	\begin{align*}
...
\begin{align*}
	...
	& = \int\limits_{\Omega} 2 T_j  \sum\limits_{i=1}^{k} v_i^h T_i u^{dis} + \nu \left( \epsilon_2 2 \sum\limits_{i=1}^{k} v_i^h \nabla T_i \nabla  T_j - \frac{2}{\epsilon_2} (1-\sum\limits_{i=1}^{k} v_i^h T_i) T_j \right) + \sum\limits_{i=1}^{k} \eta_i^h T_i T_j  \diff x \\
...
Dieses Kapitel behandelt die Implementation der Optimierung. Zunächst werde ich darauf eingehen, wie man die festen Parameter im Code \ref{sec:code} wählen muss. Dabei meint feste Parameter, dass deren Veränderung zu keinen brauchbaren Ergebnissen mehr führt. Hier sind das $c, \epsilon, \eta$ und $v_0$. Danach werde ich einige Beispiele für unterschiedliche Gitterweiten, verschiedene Risse und Möglichkeiten des Einspannens des Materials betrachten. Zum Schluss gehen wir kurz auf die Konvergenz der Methode ein.    
...
\subsubsection{$u_0$ ist konstant und der Riss ist durchgängig}
...
\paragraph{$u_0$ ist konstant und das Material ist am Rand angerissen}
