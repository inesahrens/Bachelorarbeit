...
\paragraph{Berechnung der Matrix $A_{ij}= \int\limits_{\Omega}  u^{dis} T_i T_j \diff x$}
	\begin{align*}
...
\paragraph{Berechnung der Matrix $D_{ij}= \int\limits_{\Omega}  T_i T_j \diff x$}
	\begin{align*}
...
\begin{equation*}
	(2 A + 2 \epsilon_2 B + \frac{2}{\epsilon_3} D) v  - \frac{2}{\epsilon_3} c + e=  
\end{equation*}
...
\begin{align*}
	...
	& = \int\limits_{\Omega} 2 \sum\limits_{j=1}^{k} T_j  \sum\limits_{i=1}^{k}  T_i u^{dis} + \epsilon_2 2 \nabla (\sum\limits_{i=1}^{k} T_i ) \nabla (\sum\limits_{j=1}^{k} T_j ) + \frac{2}{\epsilon_3} \sum\limits_{i=1}^{k}  T_i \sum\limits_{j=1}^{k} T_j \diff x \\
	& = 2 \sum\limits_{i,j=1}^{k} \int\limits_{\Omega} u^{dis} T_i T_j \diff x 
...
\begin{align*}
	...
	\end{array}
\end{align*}
%todo überprüfen nach richtigen 3 gesetzt
...
\subsection{Agregation}
Nun sind alle Funktionen disktretisiert und das Problem kann implementiert werden. Stellen wir die genaue Newtonmethode auf: 
