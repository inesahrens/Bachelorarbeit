...
\begin{itemize}
	...
	\item wahl von $c$ (ob egal oder nicht) falls c gaaanz groß, so bei 500, dann wird der Riss steiler, also weniger übergang
	\item Wahl von $\eta$. Was geht, was nicht?$\eta=1$ geht nichts (kein bild) $\eta=2$ riss zu steil; $\eta=.9$ riss ist nicht bei 0, geht nur bis $.15$, $\eta=.5$ riss fängt bei 0 an, geht jedoch nur bis $.3$ dann geht immer etwas höher $\eta=.34$ fast perfekt bei 1 der Rand. aber nur bei gerader riss. bei ungeraden zu niedrig. bei riss am rand und in der mitte $\eta=-.99$ gut, aber $\eta=.34$ nur wenig besser, da nicht bei 1 ankommt, eher 0.9 oder .8 hoch ist. 
	\item Wahl von $v_0$ was ergibt sinn?  Eig erwartet man, dass $v_0<1$ keinen Sinn ergibt, da dann nicht kein Riss zugelassen ist, jedoch kommen beim testen normale ergebnisse raus bis $v_0>.7$. Ob $v_0$ jetzt $1$ oder 10 gewählt ist, ist egal. -> kann man die bedingung streichen? (newtonverfahren nur nach u und v machen)
\end{itemize}
