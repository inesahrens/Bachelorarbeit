...
\begin{itemize}
	...
	\item Verifizierung Theretischer Aussagen
	\begin{itemize}
		\item Konvergiert die Newtonmethode?
		\item Beispiele dazu bringen
		\item vll auch, dass einige vorraussetzungen nicht gebraucht werden? da mal testen
	\end{itemize}
	\item justierung der Parameter
	\begin{itemize}
		\item epsilon $> gitterweite, < Gitterweite$ und warum
		\item wahl von $c$ (ob egal oder nicht)
		\item Wahl von $\eta$. Was geht, was nicht?
		\item Wahl von $v_0$ was ergibt sinn?  
		\item Wahl von $u_0$ was ergibt sinn? 
	\end{itemize}
	\item Messung der Leistungsfähigkeit der Software (schnelligkeit? einsatz?)
	\begin{itemzie}
		\item eher nicht. hat ja kaum anwendung
	\end{itemzie}
	\item einige Fallbeispiele: genügend beschreibung zum nachvollziehen/ nachmachen und interpretation
	\item testbeispiele: extremverhalten? charakteristische Probleme aus der Praxis; Testprobleme mit bekannten Ergebnissen?
	\begin{itemize}
		\item gerader riss
		\item krummer riss
		\item nur kurz angerissen
		\item unterschiedliche gitterweite
	\end{itemize}
\end{itemize}


Falls u an beiden Seiten auf unterschiedlicher aber jeweils konstanter Höhe eingespannt ist und ein Riss in der Mitte des Materials entsteht (der mind. 2 Pixel breit sein muss, sonst wird v nicht richtig 0, sondern wird interpoliert), sehen wir die Unstetigkeitsstelle genau an dem Riss. 
